...
public class ConsultarAsesores extends JDialog {
	...
	private void initDominio() {
		try{
			...
			Logger log = Logger.getLogger(ConsultarAsesores.class.getName());
	...
	public void cargaAsesores() {
		try{
			...
			for(int i=0; i<listaInmuebles.size(); i++) {
				...
				if(listaInmuebles.get(i).getAsesor().getCodigoEmp().trim().equals(ase)) {

					Object nuevo[]= {asesor.getCodigoEmp(), asesor.getNombre(), listaInmuebles.get(i).getCodId(),
