...
public class NuevaTransaccion extends JDialog {
	...
	private void initDominio() {
		try{
			...
			Logger log = Logger.getLogger(NuevaTransaccion.class.getName());
	...
	public NuevaTransaccion(Controlador control1) {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				try{
					...
					for(int i=0; i<visitas.size(); i++) {
						...
						if(visitas.get(i).getAsesor().getCodigoEmp().trim().equals(ase) && visitas.get(i).getOferta()!=null) {
		...
		visitasLabel.setBounds(10, 74, 46, 14);
		...
		fechaLabel.setBounds(10, 135, 46, 14);
		...
		JLabel visitasAsesorLabel = new JLabel("Visitas asociadas al Asesor.");
		visitasAsesorLabel.setBounds(257, 74, 167, 14);
		contentPanel.add(visitasAsesorLabel);
					...
					public void actionPerformed(ActionEvent e) {
						...
						try{
							...
							if(opcion==0) {
								...
								if(visi.getInm().getVentaAlquiler().trim().equals(vi.getCompraAlquiler().trim())) {
								...
								else if (visi.getInm().getVentaAlquiler().trim().equals("V")){JOptionPane.showMessageDialog(null, "Ese Inmueble no puede ser Alquilado,\n solo Vendido", "TRANSACCION",
										JOptionPane.DEFAULT_OPTION); 
